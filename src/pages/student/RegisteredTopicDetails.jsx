...
const RegisteredTopicDetails = ({ topic, onViewGrades, onViewCouncil }) => {
  const statusMap = {
    approved: 'Đã duyệt',
    pending: 'Chờ duyệt',
    rejected: 'Từ chối',
      ...
      <div className="flex items-start mb-6">
        ...
        {topic.topic_teacher_status === 'approved' && topic.topic_leader_status === 'approved' ? (
          <div className="flex flex-row gap-2 justify-end items-center w-full max-w-xs">
            ...
            {typeof onViewCouncil === 'function' && (
              <button
                onClick={onViewCouncil}
                className="flex items-center bg-blue-100 hover:bg-blue-200 text-blue-700 text-sm font-medium px-4 py-2 rounded transition-colors duration-300 border border-blue-300"
                style={{ minWidth: 120, justifyContent: 'center' }}
              >
                <FaUsers className="mr-2" /> Xem hội đồng
              </button>
            )}
          </div>
        ) : (
