...
import { FaUsers, FaInfoCircle, FaCheckCircle, FaTimesCircle } from 'react-icons/fa'; // Example icons
...
const ApproveGroupDetails = () => {
  ...
  const handleApprove = () => {
    ...
    alert(`Đã duyệt nhóm ${selectedGroupId} thành công (giả lập)!`);
    // TODO: Update mock data or call API to change group status to 'approved'
    // TODO: Potentially update other groups for this topic to 'rejected'
    // TODO: Update the topic status if needed
    navigate('/lecturer/approve-groups'); // Go back to the list
  };

  const handleViewInfo = (groupId) => {
     alert(`Xem chi tiết thông tin nhóm ${groupId}. Chức năng đang phát triển!`);
     // Could open a modal with more group/member details
  };
                        ...
                        {groups.map((group, index) => (
                            <tr key={group.groupId} className="hover:bg-gray-50">
                                ...
                                <td className="px-4 py-3 whitespace-nowrap text-center">
                                    <button onClick={() => handleViewInfo(group.groupId)} className="text-indigo-600 hover:text-indigo-900" title="Xem chi tiết">
